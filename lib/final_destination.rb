...
class FinalDestination
  ...
  def initialize(url, opts = nil)
    ...
    @initial_https_redirect_ignore_limit = @opts[:initial_https_redirect_ignore_limit]
  ...
  def get(redirects = @limit, extra_headers: {}, &blk)
    ...
    if result == :redirect
      ...
      if @uri && redirects == @max_redirects &&
  ...
  def resolve
    ...
    if location
      ...
      if @uri && @limit == @max_redirects && @initial_https_redirect_ignore_limit && same_uri_but_https?(old_uri, @uri)
        @initial_https_redirect_ignore_limit = false
        @limit += 1
      end
      @limit -= 1
